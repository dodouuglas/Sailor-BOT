...
const ffmpeg = require('fluent-ffmpeg') //Libwebp 
...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		//if (isGroupMsg && isCmd && !isOwner && !isBot && groupId == '556199463450-1627429855@c.us') return console.log(color('[Grupo mutado]', 'red'), color(`Mensagem do grupo mutado`, 'yellow'))
		...
		if (isBot && isCmd && chatId !== `${botNumber}@c.us`) await kill.reply(ownerNumber[1], mess.howtorun(`wa.me/+${botNumber}`), id)
		...
		if (isGroupMsg && isAntiTravas && isTrava && !isGroupAdmins && isBotGroupAdmins && !isOwner && oneTrava == 0 && !isBot) {
			try {
				...
				//await kill.sendText("559391546825-1519748070@g.us", mess.recTrava(user) + `\nAt/No > ${name}`).then(async () => { await kill.contactBlock(user) }) // Avisa o dono do bot e bloqueia o cara
				await kill.setGroupToAdminsOnly(groupId, false);return oneTrava = 0 // Reabre o grupo
			} catch (error) { return oneTrava = 0 }
		}

		if (!isGroupMsg && !isCmd) {
			const theMsgg = await gaming.getValue(user, nivel, 'msg')
			const uzrXpp = await gaming.getValue(user, nivel, 'xp')
			const uzrlvll = await gaming.getValue(user, nivel, 'level')
			const mpingg = processTime(t, moment())
    		 isMsg[user] == null ? isMsg[user] = 1 : isMsg[user] += 1
    		 if (isMsg[user] < 2) {
				await kill.sendText(from, mess.menu(pushname, time, theMsgg, uzrXpp, getReqXP(uzrlvll), uzrlvll, mpingg, patente), id)
    		 }
		}
		
		// Bloqueia travas no PV
		if (!isGroupMsg && !isOwner && isTrava && !isBot) { await kill.contactBlock(user).then(async () => { await kill.sendText(ownerNumber[1], mess.recTrava(user)) }) }
		...
		if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && !isOwner && oneTrava == 0 && !isBot) {
			try {
				if (chats.length > Number(config.Max_Characters)) {
					...
					//await kill.sendText("559391546825-1519748070@g.us", mess.recTrava(user)).then(async () => { await kill.contactBlock(user);return oneTrava = 0 }) // Avisa o dono e então bloqueia a pessoa
				}
			} catch (error) { return oneTrava = 0}
		}
		...
		if (!isGroupMsg && !isOwner && !isBot) {
			try {
				...
				//	return await kill.contactBlock(user).then(async () => { await kill.sendText("559391546825-1519748070@g.us", mess.recTrava(user)) }) // Avisa o dono e bloqueia
				}
			} catch (error) { return }
		}
		...
		if (!isFiltered(from) && !isMedia && !isCmd && !isBlocked) { try { if (chats.includes(`@${botNumber.replace('@c.us', '')}`)) { await kill.reply(from, chatBotR, id) } } catch (error) { return } }


		//if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes(`@${botNumber.replace('@c.us', '')}`)) { await kill.sendPtt(from, './bot.mp4', id) } } catch (error) { return } }
		
		// Caso deseje criar siga o estilo disso abaixo, para usar a base remova a /* e a */
		
		//if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('bem')) await kill.sendPtt(from, './bem-ti-vi.mpeg', id) } catch (error) { return console.log(error) } }

		//if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('bot')) await kill.sendPtt(from, './bot.mp4', id) } catch (error) { return console.log(error) } }

		//if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('cu')) await kill.sendPtt(from, './cu.mp4', id) } catch (error) { return console.log(error) } }
		

		//if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('🤣')) await kill.reply(from, 'KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK quem usa emogi em 2K21', id) } catch (error) { return console.log(error) } }
		//if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('😂')) await kill.reply(from, 'KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK quem usa emogi em 2K21', id) } catch (error) { return console.log(error) } }

		// Se falhar você pode tentar chats.toLowerCase().includes 
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('agiota')) await kill.sendFile(from, './random.mp4', 'random.mp4', 'clb random', id) } catch (error) { return } }

		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('pix')) await kill.sendText(from, `Em falar em pix sera que você poderia mandar para mim?\nsailorbotwpp@gmail.com`, id) } catch (error) { return } }

		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('sexo') || chats.toLowerCase().includes('sexo')) await kill.sendPtt(from, './sexo.mp4', id) } catch (error) { return console.log(error) } }

		if (!isFiltered(from) && !isMedia && !isCmd && !isBlocked) { 
			try { 
				if (chats.toLowerCase().includes('batatinha frita 123')) {
					await kill.sendImageAsSticker(from, './lib/media/figurinhas/foto(1)1.png') 
					await kill.sendImageAsSticker(from, './lib/media/figurinhas/foto(2)1.png')
					await kill.sendImageAsSticker(from, './lib/media/figurinhas/foto(3)1.png')
					await kill.sendImageAsSticker(from, './lib/media/figurinhas/foto(4)1.png')
					await kill.sendButtons(from, "Clique no butão", [{id: "1", "text": 'Quero!!'}], `Quer mais figurinhas?`)
				}
			} catch (error) { return } 
		}
		...
		//const mutegrupos = ['559391546825-1529748070@g.us']
		//if (isCmd && !isOwner && groupId == `${mutegrupos}`) return console.log(color('> [SILENCE]', 'red'), color(`Ignorando comando de ${pushname} porque estar mutado`, 'yellow'))

		// Ignora pessoas bloqueadas 
		if (isBlocked && isCmd && !isOwner) {
			await kill.reply(from, `Você foi banido de usar meus comandos caso ache injusto fale com o meu dono\n Agiota(Dono)- https://wa.me/558581525760`, id)
			return console.log(color('> [BLOCK]', 'red'), color(`Ignorando comando de ${pushname} - [${user.replace('@c.us', '')}] por ele estar bloqueado...`, 'yellow'))
		}

		// Anti Flood para PV'S
		if (isCmd && isFiltered(from) && !isGroupMsg && !isOwner) { 
			await gaming.addValue(user, Number(-100), nivel, 'xp');
			await kill.reply(from, "*⚠ [Anti-Flood] ⚠*\n Espere 10 seguntos para dar outro comando\nVocê perdeu 100 *[XP]*", id) 
		...
		// Mensagens no PV
		if (!isCmd && !isGroupMsg) { try { console.log(`\nMensagem > "${chats}"`) } catch (e) {} return console.log('> MENSAGEM AS', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'DE', color(`"${pushname} - [${user.replace('@c.us', '')}]"`)) }

		// Mensagem em Grupo
		if (!isCmd && isGroupMsg) { try { console.log(`\nMensagem > "${chats}"`) } catch (e) {} return console.log('> MENSAGEM AS', color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'DE', color(`"${pushname} - [${user.replace('@c.us', '')}]"`), 'EM', color(`"${name || formattedTitle}"`)) }
		...
		//if (isCmd && isGroupMsg && isOwner) { kill.sendText(ownerNumber[0], `> COMANDO "[${prefix}${command.toUpperCase()}]" DE "${pushname} - [${user.replace('@c.us', '')}]"`) }

		/*const bott = await axios.get(`http://simsumi.herokuapp.com/api?text=${chat}&lang=${region}`)
		const pvsss = ['558399790592@c.us', '5522999783699@c.us', '556295515401@c.us', '555193768151@c.us', '5522991055315@c.us', '558581525760@c.us']
		if (!isCmd && pvsss.includes(user)) { 
			try { 
				if (bott.data.success == 'Limit 50 queries per hour.' || bott.data.success == '' || bott.data.success == null) {
					await kill.sendText(from, chatBotR, id)
				} else {
					if (bott.data.success == 'Curta a pagina Gamadas por Bieber no facebook ;)') return await kill.sendText(from, 'Oi sua gostosa, como vai?', id)
					await kill.sendText(from, bott.data.success, id)
		...
		switch(command) {
				...
				if(ar[0] == 'add') {
					if(mentionedJidList.length !== 0) {
						for (let prem of mentionedJidList) {
							if(prem == botNumber) return await kill.reply(from, `Formato incorreto!\nO formato correto é...\n${prefix}vip add @user dia(s)`, id)
							premium.addPremiumUser(prem, args[2], _premium)
							await kill.reply(from,`*--- [ VIP ADICIONADO ] ---*\n\n *ID*: ${prem}\n *Expira em*: ${ms(toMs(args[2])).days} dia(s) ${ms(toMs(args[2])).hors} Hora(s) ${ms(toMs(args[2])).minutes}`, id)
						}
					} else {
					...
					if(mentionedJidList.length !== 0) {
						if(mentionedJidList[0] == botNumber) return await kill.reply(from, `Formato incorreto!\nO formato correto é...\n${prefix}vip del @user dia(s)`, id)
						_premium.splice(premium.getPremiuumPosition(mentionedJidList[0], _premium), 1)
						fs.writeFileSync('./lib/config/gerais/vips.json', JSON.stringify(_premium))
						await kill.reply(from, `Feito!`, id)
					} else {
						_premium.splice(premium.getPremiuumPosition(args[1] + '@c.us', _premium), 1)
						fs.writeFileSync('./lib/config/gerais/vips.json', JSON.stringify(_premium))
						await kill.reply(from, `Feito!`, id)
					}
				} else {
					await kill.reply(from, `Formato errado!`, id)
				}
				break*/

			/*case 'comprar':
					try {
						if(!user) return await kill.reply(from, mess.registro(), id)
						if(isGroupMsg) {
							var nomevip = quotedMsg ? quotedmsgObj.sender.id : (mentionedJidList.length !== 0 ? menUser.id : user)
							const buyvip2 = await gaming.getValue(user, nivel, 'money')
							if(buyvip2 <= 50000) return await kill.reply(from, mess.coinssufic7(buyvip2), id)
							if (vips.includes(user)) return await kill.reply(from, 'sla', id)
							if (quotedMsg) {
								vips.push(user)
								await fs.writeFileSync('./lib/config/gerais/vipuser.json', JSON.stringify(vips))
								await kill.reply(user, mess.buyviptime(nomevip, vips), id)
								await gaming.addValue(user, Number(-50000), nivel, 'money')
								await kill.sendText(ownerNumber[1], mess.buyviplog(user))
								await sleep(1000)
								console.log(color('COMPROU-VIP-TIME'), 'yelloy'), color(`Nome: ${pushname}`, 'yellow')
							} else {
								vip.push(user)
								await fs.writeFileSync('./lib/config/gerais/vipuser.json', JSON.stringify(vips))
								await kill.reply(user, mess.buyviptime(nomevip, vips), id)
								await sleep(1000000)
								await kill.reply(user, mess.timevipend(nomevip), id)
								await kill.sendText(ownerNumber[1], mess.vipendlog(user));
								console.log(color('VIP TIME UP END'), 'yelloy'), color(`Nome: ${pushname}`, 'red')
								const pos = vips.indexOf(user)
								vips.splice(pos, 1)
								await fs.writeFileSync('./lib/config/gerais/vipuser.json', JSON.stringify(vips))
							}
						} else if (isGroupMsg) {
							await kill.reply(from, mess.soadeniro(), id)
						} else return await kill.reply(from, mess.sogrupo(), id)
					} catch (error) {
						console.log(color('[TIME UP'), 'crimson'), color(`=> Obtive error no comando ${prefix}${command} => ${error.message} - voce pode ignorar.`, 'gold'))
					}
			break*/
			...
			case 'glink':
				function gerar_aleatorio(length) {
					var resultado = '';
					var caracters =
				'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
					var quantidade_caracters = caracters.length;
					for ( var i = 0; i < length; i++ ) {
					  resultado += caracters.charAt(Math.floor(Math.random() * quantidade_caracters));
				   }
				   return resultado;
				}	
				await kill.sendText(from, `https://chat.whatsapp.com/${gerar_aleatorio(22)}`, id)
				break	

			case 'qrpix':
					if (args[0].toLowerCase() == '-telefone') {
					if(args.length == 0) {
						await kill.reply(from, 'Informe sua chave pix numero de telefone!', id)
					} else
					if(args.length > 11) {
						await kill.reply(from, 'USE APENAS CHAVE PIX NUMERO DE TELEFONE', id)
					} else
					await kill.reply(from, mess.wait(), id)
						await kill.sendFileFromUrl(from, `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=00020126360014BR.GOV.BCB.PIX0114+55${args[1]}5204000053039865802BR5903sla6003sla62070503***6304B772`, id)
					} else if (args[0].toLowerCase() == '-cpf') {
					if(args.length == 0) {
						await kill.reply(from, 'Informe sua chave pix cpf!', id)
					} else
					if(args.length > 11) {
						await kill.reply(from, 'USE APENAS CHAVE PIX CPF', id)
					} else
					await kill.reply(from, mess.wait(), id)
						await kill.sendFileFromUrl(from, `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=00020126330014BR.GOV.BCB.PIX0${args[1]}5204000053039865802BR5903sla6003sla62070503***63042D6C`, id)
					} else if (args[0].toLowerCase() == '-chavealeatoria') {
					if(args.length == 0) {
						await kill.reply(from, 'Informe sua chave pix chave aleatoria de telefone!', id)
					} else
					if(args.length > 36) {
						await kill.reply(from, 'USE APENAS CHAVE PIX CHAVE ALEATORIA', id)
					} else
					await kill.reply(from, mess.wait(), id)
						await kill.sendFileFromUrl(from, `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=00020126580014BR.GOV.BCB.PIX0136${args[1]}5204000053039865802BR5903sla6003sla62070503***6304D516`, id)
					} else if (args[0].toLowerCase() == '-help') {
						await kill.reply(from, `COMO USAR O GERADOR DE QR PIX:\n\n!qrpix -cpf\n!qrpix -telefone \n!qrpix -chavealeatoria`)
					}
					else {
						await kill.reply(from, 'Use !qrpix -help')
					}
					break				

			case 'butao': 
				if (args.length == 0) return await kill.reply(from, mess.noargs() + '!butao vc namoraria cmg?\n ele sempre vai colocar sim ou não.', id)
				await kill.sendButtons(from, `${body.slice(7)}`, [{id: "999", "text": 'sim'}, {id: "100", "text": 'não'}], ``) 
				break		
				...
				case 'fotop':
					...
					const novafotop = await fs.readFileSync(`./lib/media/fotos/foto(${getrphoto()}).png`)
					await kill.setProfilePic(`data:image/png;base64,${novafotop.toString('base64')}`)
					...
					if (!isOwner) return await kill.reply(from, mess.sodono(), id)
					if (!fs.existsSync(`./lib/config/Utilidades/lastchanged.json`)) { await fs.writeFile(`./lib/config/Utilidades/lastchanged.json`, '[{"profile":[],"group":[]}]') }
					const novafotog = await fs.readFileSync(`./lib/media/fotos/foto(${getrphoto()}).png`)
				//	await kill.setGroupIcon('559391546825-1519748070@g.us', `data:image/png;base64,${novafotog.toString('base64')}`, `foto(${getrphoto()}).png`)
			...
			case 'ttp':
				...
				await axios.get(`https://api.xt./eam.xyz/ttp?file&text=${encodeURIComponent(body.slice(5))}`, { responseType: 'arraybuffer' }).then(async (response) => {
			...
			case 'calcular':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + 'número & simbolos matematicos.', id)
				const mtk = body.slice(9)
				try {
			...
			case 'water':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + 'palavras/números.', id)
				try {
					if (arks.length >= 16) return await kill.reply(from, 'Max: 10 letras.', id)
			...
			case 'setimage':
				...
				case 'deleted':
				if (!isOwner) return await kill.reply(from, mess.sodono(), id)
				if (arks.includes('-exclude')) {
					deleted[0].exceptions.push(from)
					console.log(deleted[0].exceptions)
				...
				if (arks.includes('-include') && deleted[0].exceptions.includes(from)) {
					let pos = deleted[0].exceptions.indexOf(from)
					deleted[0].exceptions.splice(pos, 1)
					await fs.writeFileSync('./lib/config/Utilidades/deleted.json', JSON.stringify(deleted))
					return await kill.reply(from, mess.enabled(), id)
				}
				var msgdels = 'Mensagens apagadas:\n\n'
				for (let i = 0; i < deleted[0].msgs.length; i++) {
					var contato = await kill.getContact(deleted[0].msgs[i].from)
					var nome = contato.pushname == null ? 'wa.me/' + deleted[0].msgs[i].from.replace('@c.us', '') : contato.pushname
					var contato2 = await kill.getContact(deleted[0].msgs[i].to)
					var nome2 = contato2.pushname == null ? 'wa.me/' + deleted[0].msgs[i].to.replace('@c.us', '') : contato2.pushname
					msgdels += 'De: ' + nome + '\n'
					msgdels += 'Para: ' + nome2 + '\n'
					msgdels += 'Quando: ' + deleted[0].msgs[i].t + '\n'
					msgdels += 'Mensagem: ' + deleted[0].msgs[i].body + '\n\n'
				}
				await kill.reply(from, msgdels, id)
				break
			...
			/*case 'sexo':
				if (args.length == 0) return await kill.sendText(from, mess.noargs() + 'instagram usernames.')
				const igg = await axios.get(`https://www.instagram.com/${encodeURIComponent(body.slice(5))}/?__a=1`)
				const stkigg = JSON.stringify(igg.data)
				if (stkigg == '{}') return await kill.sendText(from, mess.noresult())
				await kill.sendFileFromUrl(from, `${ig.data.graphql.user.profile_pic_url}`, ``, `nome: ${}\nDescrição: ${}\nQuantidade de Fotos: ${}`)
				break*/

			case 'freefire':
				if (args.length == 0) return await kill.sendText(from, 'Use o id')
				if(!isNaN(`${encodeURIComponent(body.slice(9))}`)) return await kill.sendText(from, 'Use o apenas numeros')
				const freefire = await axios.get(`https://api.xteam.xyz/search/freefire?id=${encodeURIComponent(body.slice(9))}&APIKEY=9616ea8b1fb97f6f`)
				await kill.sendText(from, `ID: ${freefire.data.result.id}\nNome: ${freefire.data.result.name}`)
				break
			...
			case 'video':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + 'Títulos do YouTube.', id)
				try {
					await kill.reply(from, mess.wait(), id)
			...
			case 'quote':
				if (args.length <= 1 || !arks.includes('|')) return await kill.reply(from, mess.noargs() + 'palavras/números.' + '\n\n' + mess.argsbar() + 'use 1 "|".', id)
				await kill.reply(from, mess.wait(), id)
		   ...
		   case 'translate':
			...
			case 'tts':
				if (args.length <= 1) return await kill.reply(from, mess.noargs() + 'palavras/números.', id)
				try {
			...
			case 'criador':
				await kill.reply(from, `☀️ Agiota(Dono)- https://wa.me/558581525760`, id)
				break
			...
			case 'welcome':
				...
				case 'antimsg':
					...
					if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
						if (args.length !== 1) return await kill.reply(from, mess.onoff(), id)
						if (args[0].toLowerCase() == 'on') {
							if (functions[0].antimsg.includes(groupId)) return await kill.reply(from, mess.jaenabled(), id)
							functions[0].antimsg.push(groupId)
							await fs.writeFileSync('./lib/config/Gerais/functions.json', JSON.stringify(functions))
							await kill.reply(from, mess.enabled(), id)
						} else if (args[0].toLowerCase() == 'off') {
							if (!functions[0].antimsg.includes(groupId)) return await kill.reply(from, mess.jadisabled(), id)
							functions[0].antimsg.splice(groupId, 1)
							await fs.writeFileSync('./lib/config/Gerais/functions.json', JSON.stringify(functions))
							await kill.reply(from, mess.disabled(), id)
						} else return await kill.reply(from, mess.kldica1(), id)
					} else if (isGroupMsg) {
		   ...
		   case 'clima':
			...
			case 'profile':;case 'perfil':
				if (isGroupMsg) {
					...
					if (pic == 'ERROR: 401' || typeof pic === 'object') { var pfp = errorurl } else { var pfp = pic }
			...
			case 'store':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + 'Nome do App.', id)
				await kill.reply(from, mess.wait(), id)
				await sleep(5000)
			...
			case'bcc':
				...
				break
							...
							case'bcc2':
							if (!isOwner[1] && !isOwner[0]) return await kill.reply(from, mess.sodono(), id)
							if (args.length == 0) return await kill.reply(from, mess.broad(), id)
							const chatalll = await kill.getAllChatIds()
							const isGroupCC = await chatalll.filter(group => group.includes('@g.us'))
							const isPrivateCC = await chatalll.filter(privat => privat.includes('@c.us'))
							try {
								const sendQFileC = async (quotedMsgObj, ids) => {
									let replyOnReply = await kill.getMessageById(quotedMsgObj.id)
									let obj = replyOnReply.quotedMsgObj
								...
								if (args[0].toLowerCase() == '-all') {
									for (let ids of chatalll) {
										var cvk = await kill.getChatById(ids)
										if (!cvk.isReadOnly) {
											try {
												await kill.sendText(ids, `${msg}`)
												if (quotedMsgObj) { await sendQFileC(quotedMsgObj, ids) }
											} catch (error) { console.log(color('[BROADCAST]', 'crimson'), color(`→ Uma das mensagens não foi enviada - Você pode ignorar.`, 'gold')) }
										}
									}
			...
			case 'sip':
				...
				await kill.sendFile(from, `./lib/media/img/${user.replace('@c.us', '')}ip.png`, 'ip.png', 'Talvez aqui! 📍', id)
				await sleep(10000).then(async () => { await fs.unlinkSync(`./lib/media/img/${user.replace('@c.us', '')}ip.png`) })
				break
				
			case 'scep':
				if (!region == 'pt') return await kill.reply(from, 'Brasil solamente!', id)
			...
			case 'scpf':
				if (!region == 'pt') return await kill.reply(from, 'Brasil solamente!', id)
				if (args.length !== 1) return await kill.reply(from, mess.noargs() + 'cpf (ex: 00022988130).', id)
				const cpf = await axios.get(`https://607387dae4e0160017ddfad3.mockapi.io/api/ConsultaCPF/dados/${encodeURIComponent(body.slice(6))}`)
				await kill.reply(from, mess.scpf(cpf), id)
				break				
		...
		case 'admsbot':
			let hehe = `═✪〘 🖊️ - Todos os ADMS do bot- 🐂 〙✪═\n\n`
			for (let i = 0; i < ownerNumber.length; i++) { hehe += `- @${ownerNumber[i].replace('@c.us', '')}\n` }
			hehe += '\n═✪〘 Todos Os ADMS 〙✪═'
			await sleep(2000)
			await kill.sendTextWithMentions(from, hehe)	
			break	
			...
			case 'arquivar2':
					if (isGroupMsg) {
					const groupOwnerzz = user === chat.groupMetadata.owner
					for (let noadms of groupAdmins) {
						if (noadms == groupOwnerzz || ownerNumber.includes(noadms) || noadms == botNumber) {
							console.log(color('[VIP] - ', 'crimson'), noadms)
						} else { await kill.demoteParticipant(groupId, noadms) }
					}
					await kill.setGroupToAdminsOnly(groupId, true)
					await kill.setGroupIcon(groupId, './arquivadu.jpg')
						if (isOwner) {
							for (let i = 0; i < groupMembers.length; i++) { 
			...
			case 'clear':
				...
				for (let dchat of allChatz) { await kill.deleteChat(dchat.id) } // Se quiser deletar os chats, use 'await kill.deleteChat(dchat.id)', mas cuidado com fake, welcome, blacklist e outros.
				await kill.reply(from, mess.maked(), id)
				break
			...
			case 'wolf':
				if (args.length >= 2 && arks.includes('|')) {
					...
					if (arg.split('|')[0].length >= 10 || arg.split('|')[1].length >= 10) return await kill.reply(from, 'Max: 10 letras p/frase - phrase.', id)
					await kill.reply(from, mess.wait() + '\n\n20+ s.', id)
				...
				} else return await kill.reply(from, mess.noargs() + 'palavras/números.' + '\n\n' + mess.argsbar() + 'use 1 "|".', id)
				break
			...
			case 'retro':
				if (args.length >= 4 && arks.includes('|')) {
					if (arg.split('|')[0].length >= 10 || arg.split('|')[1].length >= 10 || arg.split('|')[2].length >= 10) return await kill.reply(from, 'Max: 10 letras p/frase - phrase.', id)
					await kill.reply(from, mess.wait() + '\n\n20+ s.', id)
			...
			case 'kick':;case 'ban':
				if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
					...
					return await kill.reply(from, mess.sogrupo(), id)
				break
			...
			case 'sair':
				if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
					...
					await sleep(1000)
					await kill.leaveGroup(groupId) 
					await sleep(1000)
					await kill.deleteChat(groupId)
				} else if (isGroupMsg) {
					await kill.reply(from, mess.soademiro(), id)
				} else return await kill.reply(from, mess.sogrupo(), id)
				break
				...
				case 'sair2':
					if (isOwner) {
						const grupo = args[0]
						await kill.sendText(`${grupo}@g.us`, mess.goodbye()).then(async () => { await kill.leaveGroup(`${grupo}@g.us`)})
					}
					break				
			...
			case 'join':
				...
				if (tGr.length > 25) return await kill.reply(from, mess.cheio(tGr), id)
				if (memberlmt < 15) return await kill.reply(from, mess.noreq(memberlmt), id)
				if (check.status == 200) {
			...
			case 'phcom':
				try {
					if (args.length == 0 || !arks.includes('|')) return await kill.reply(from, mess.noargs() + 'palavras/números.' + '\n\n' + mess.argsbar() + 'use 1 "|".', id)
					await kill.reply(from, mess.wait(), id)
					if (isImage || isQuotedImage) {
			...
			case 'pix':
				await kill.reply(from, '52e6d786-73c9-4b36-915c-7e816e96dd2c', id)	
				break

			case 'regras':
				await kill.reply(from, mess.regras(pushname), id)	
				break

			case 'premium':
				await kill.reply(from, mess.novogrupo2(), id)
				break	
			...
			case 'mutepv':
				...
				if (isOwner) {
					if (args[0].toLowerCase() == 'on') {
						...
						await fs.writeFileSync('./lib/config/Gerais/silencepv.json', JSON.stringify(functions))
						await kill.reply(from, mess.enabled(), id)
					} else if (args[0].toLowerCase() == 'off') {
						const pvmt = body.slice(11) + '@c.us'
						if (!functions[0].silence.includes(pvmt)) return await kill.reply(from, mess.jadisabled(), id)
			...
			case 'help':
				...
				await kill.sendText(ownerNumber[1], mess.advise(nopvne, isgorp, user, body, hpgp))
			...
			case 'cosplay':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + 'nome.', id)
			...
			case 'eval':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + `JS Código.`, id)
				if (!isOwner) return await kill.reply(from, mess.sodono(), id)
				try { eval(body.slice(6)) } catch (error) { kill.sendText(from, `→ Obtive erros no comando ${prefix}${command} → ${error.message}`) }
				break
			...
			default:
				if (isCmd) {
					...
					await exec(`cd lib/config/Utilidades && bash -c 'grep -i "${command.slice(0, 3)}" comandos.txt | shuf -n 1'`, async (error, stdout, stderr) => {
						if (error || stderr || stdout == null || stdout == '') {
						 			await kill.sendButtons(from, "Para ver meus comandos clique no butao.", [{id: "666", "text": '!menu'}], `⚠️ - O comando ${prefix}${command} não existe`)
